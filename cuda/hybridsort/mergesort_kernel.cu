...
__global__ void
mergeSortFirst(const float4 *source, float4 *result, int listsize) 
{
    // Block index
    int bx = blockIdx.x;
    // Thread index
    //int tx = threadIdx.x;
		if(bx*blockDim.x + threadIdx.x < listsize/4){
			float4 r = source[bx*blockDim.x + threadIdx.x];
			result[bx * blockDim.x + threadIdx.x] = sortElem(r); 
		}
}

__global__ void
mergeSortPass(const float4 *source, float4 *result, int nrElems, int threadsPerDiv) 
{
	...
	int outidx = 0; 
	float4 a, b;
	a = source[Astart + aidx];  
	b = source[Bstart + bidx]; 
	...
	while(true)//aidx < nrElems/2)// || (bidx < nrElems/2  && (Bstart + bidx < constEndAddr[division])))
	{
		 ...
		 * after the merge
		 */
		float4 nextA = source[Astart + aidx + 1]; 
		float4 nextB = source[Bstart + bidx + 1]; 
